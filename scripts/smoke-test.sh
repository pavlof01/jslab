...
API_KEY="${JSLAB_API_KEY:-}"
CURL_FLAGS="${CURL_FLAGS:-}"

BASE_URL="${BASE_URL%/}"

if [[ -z "$API_KEY" ]]; then
  echo "JSLAB_API_KEY is required for /api/run smoke test." >&2
  exit 1
fi
...
echo "Checking /api/healthz without key..."
health_headers="$tmp_dir/health.headers"
health_body="$tmp_dir/health.body"
health_status="$(curl -sS $CURL_FLAGS -D "$health_headers" -o "$health_body" -w "%{http_code}" "$BASE_URL/api/healthz")"
expect_status "$health_status" "401" "/api/healthz"

if grep -Eiq "^content-type:.*text/html" "$health_headers"; then
  echo "/api/healthz returned HTML content-type." >&2
  exit 1
fi

if grep -Eiq "<!doctype html|<html" "$health_body"; then
  echo "/api/healthz returned HTML body (likely routed to frontend)." >&2
  exit 1
fi

if grep -Eiq "^x-powered-by:.*next\.js" "$health_headers"; then
  echo "/api/healthz returned Next.js headers." >&2
  exit 1
fi

if grep -Eiq "^x-nextjs-" "$health_headers"; then
  echo "/api/healthz returned Next.js headers." >&2
  exit 1
fi

if grep -Eiq "^server:.*next" "$health_headers"; then
  echo "/api/healthz returned Next.js server header." >&2
  exit 1
fi

echo "Checking /api/run with key..."
