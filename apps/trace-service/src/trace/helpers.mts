...
export function getValue(arg: Value) {
  if ('value' in arg && typeof arg.value !== 'object') {
    return arg.value;
  }
  if (arg instanceof UndefinedValue) return undefined;
  if (arg instanceof NullValue) return null;
  return arg.type; // For Symbol and Object
}
 ...
 *
 * Works with any generator function that yields values and returns a final result,
 * such as ToNumber(value), ToString(value), ToBoolean(value), etc.
 *
 * @param generator - Any generator function result
 * @returns The final value with trace information preserved
 *
 * @example
 * const numberResult = callGenerator(ToNumber(value));
 * const stringResult = callGenerator(ToString(value));
 * const boolResult = callGenerator(ToBoolean(value));
 */
export function callGenerator(generator: Generator) {
 ...
 *
 * @param toNumberGenerator - Result of calling ToNumber(value)
 * @returns NumberValue with trace information
 */
