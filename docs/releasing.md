...
| --- | --- |
| The repo / a release | git tag `vX.Y.Z`, `.release-please-manifest.json`, root `package.json` |
...
- `RELEASE_PLEASE_TOKEN` repo secret: a fine-grained PAT (contents: write,
  ...
  be merged.
- The first release PR after adoption proposes a bump from `0.1.0`. To open
  the public history at a different number, land an empty commit whose body
  carries `Release-As: 1.0.0` and let the bot re-cut.
