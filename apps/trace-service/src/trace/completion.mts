...
import { skipDebugger } from './index.mts';
...
class CompletionImpl<const T> {
  ...
  constructor(init: CompletionInit<T>) {
    // Check if we have a Type property on the prototype
    // If new.target.prototype doesn't have Type defined, we're being called directly (not from a subclass)
    // and need to delegate to the appropriate subclass
    const targetProtoType = (new.target.prototype as any).Type;
    if (targetProtoType === undefined) {
      // We're being called from a direct invocation (new Completion or proxy wrapper)
      // Delegate based on init.Type
      switch (init.Type) {
        case 'normal':
...
export function evalQ<T>(callback: (q: typeof Q, x: typeof X) => T | Promise<T>): Promise<NormalCompletion<T> | ThrowCompletion> | NormalCompletion<T> | ThrowCompletion {
  ...
  // Implementation for X that handles values and generators
  const X_wrapper = (value: any): any => {
    // Handle generators (yield* operations that weren't yielded)
    if (typeof value === 'object' && value && 'next' in value) {
      let result = value.next() as any;
      while (!result.done) {
        try {
    ...
    if (value instanceof Completion) {
      ...
      // Normal completion - return the wrapped value
      if (value instanceof NormalCompletion) {
        return value.Value;
      }
      return value.Value;
    }
    
    // Return the value as-is (could be void, object, etc.)  
    return value;
  };
  
  try {
    // Monkey-patch Q_impl and X_impl for the duration of the callback
    Q_impl = Q_runtime;
    X_impl = X_wrapper as any;
